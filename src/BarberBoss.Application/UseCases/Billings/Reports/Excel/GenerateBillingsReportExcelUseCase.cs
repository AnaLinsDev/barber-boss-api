...
public class GenerateBillingsReportExcelUseCase : IGenerateBillingsReportExcelUseCase
{
    ...
    public async Task<byte[]> Execute(DateOnly month)
    {
        ...
        foreach (var billing in billings)
        {
            ...
            worksheet.Cell($"C{raw}").Value = billing.Amount;
            worksheet.Cell($"C{raw}").Style.NumberFormat.Format = $"- {CURRENCY_SYMBOL} #,##0.00";

            worksheet.Cell($"D{raw}").Value = billing.Notes;
    ...
    private void InsertHeader(IXLWorksheet worksheet)
    {
        ...
        worksheet.Cell("D1").Value = ResourceReportGenerationMessages.NOTES;

        worksheet.Cells("A1:D1").Style.Font.Bold = true;
        worksheet.Cells("A1:D1").Style.Fill.BackgroundColor = XLColor.FromHtml("#F5C2B6");
